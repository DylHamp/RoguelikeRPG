...
//build library of available weapons
WEAPONS["BASIC_SWORD"] = {name: "DEFAULT_SWORD", type: 0, attack: 5};
...
function readInWeapons()
{
    ...
    for (let thing in fields)
    {
        ...
        WEAPONS[values[0]] = {name: values[0], type: Number(values[1]), attack: Number(values[2]), accuracy: Number(values[3]), crit: Number(values[4]), weight: Number(values[5]), price: Number(values[6])};
    }
}
...
function readInArmors()
{
    ...
    {
        let values = fields[thing].split(",");
        ARMORS[values[0]] = {name: values[0], type: Number(values[1]), defence: Number(values[2])};
    }
}